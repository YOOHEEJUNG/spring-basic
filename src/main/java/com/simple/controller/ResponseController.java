...
public class ResponseController {
	...
	// 스프링에서 redirect는 다시 컨트롤러를 태우는 요쳥이다
	...
	public String login(@RequestParam("id") String id, 
		...
		//로그인 성공 - home화면으로 보내버리자
		if(id.equals(pw)) {
			
			ra.addFlashAttribute("msg", "어서와 스프링은 처음이지?"); //일회성 데이터
			return "redirect:/"; //다시 home 컨트롤러를 태워 보냄
			
		//로그인 실패 - 
		}else {
			ra.addFlashAttribute("msg", "정신차려!! 이 각박한 세상 속에서");
